...
(function() {
    ...
    if (centerBody) {
         ...
         * @param {func} modeFunc 'convertScore2NCUGP' or 'convertScore2NTUGP'.
         */
        ...
        selectSeniorJuniorBtn.innerHTML = 'Select All Senior/Junior Courses';
        ...
        selectLast60Btn.onclick = function() {
            ...
            while (totalCredits < 60 && i >= 0) {
                if (isValidScore(scoreCol[i]) &&
                    ...
                    totalCredits += Number(scoreCol[i].children[3].innerHTML);
                    scoreCol[i].classList.add('selected');
                }
                i--;
            }
        ...
        selectSeniorJuniorBtn.onclick = function() {
            deselectAll();
            ...
            for (let i = 0; i < scoreCol.length; i++) {
                if (isValidScore(scoreCol[i]) &&
                    !scoreCol[i].classList.contains('selected') &&
                    Number(scoreCol[i].firstElementChild.innerHTML.slice(0, -1))
                    - firstYear >= 2) {
                    scoreCol[i].classList.add('selected');
                }
            }
        ...
        $('label[for="ModeNCU(4.0)"').click(function() {
            insertGpaTableData(convertScore2NCUGP);
        });
        $('label[for="ModeNTU(4.3)"').click(function() {
